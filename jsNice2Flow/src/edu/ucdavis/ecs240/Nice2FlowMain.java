...
public class Nice2FlowMain {
    ...
    public void outputNewFile(String filename) {
        PrintWriter writer = null;
        try {
            writer = new PrintWriter(filename.substring(0, filename.length() - 3) + "_nice2flow.js", "UTF-8");
    ...
    protected void processLine(String content, int line) {
        ...
        if (content.contains(TAG_PARAM)) {
            ...
            if(this.progMap.containsKey(KEY_RET) && !this.progMap.get(KEY_RET).getVarType().equals(NiceType.UNDEF)) {
                sb.append(": ").append(this.progMap.get(KEY_RET).getVarType());
            }
            sb.append(" {");
